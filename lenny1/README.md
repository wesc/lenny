...
- `knowledge/`: persistent knowledge store with two subdirectories:
  - `knowledge/comprehensions/`: distilled knowledge generated by compaction — LLM-generated summaries with citations to source files in `knowledge/references/`
  - `knowledge/references/`: archive of compacted files and immutable context accessed via tool calls (e.g. `knowledge/references/matrix/`, `knowledge/references/chats/`, `knowledge/references/turns/`)
...
When `dynamic/` grows beyond `max_context_tokens` (default 4000), the dreamer runs compaction:
...
4. Atomic-write the comprehension to `knowledge/comprehensions/{timestamp}-compact.md`
...
- `dream` — Watch `system/`, `dynamic/`, and `knowledge/` for changes, run background actions (e.g. sessionize chats), and compact `dynamic/` when it exceeds the token threshold.
